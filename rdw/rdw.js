...
async function init() {
  const data = await fs.promises.readFile(path.dirname(require.main.filename) + '/rdw/gebreken.json');
  const decoded = JSON.parse(data);
  for (const gebrek of decoded) {
...
function generateURLAPKBasedOnLicenseplate(licenseplate) {
  return urlAPK+encodeURI(util.format(queryAPK, licenseplate));
}

const url = 'https://opendata.rdw.nl/resource/m9d7-ebf2.json?$query=';
const query = `SELECT
  \`kenteken\`,
  \`voertuigsoort\`,
  \`merk\`,
...
function generateURLBasedOnLicenseplate(licenseplate) {
  return url + encodeURI(util.format(query, licenseplate));
}

async function fetchLicenseplate(licenseplate) {
  await ready;
  const vehicleURL = generateURLBasedOnLicenseplate(licenseplate);
  const res = await fetch(vehicleURL);
  const info = await res.json();

  const apkURL = generateURLAPKBasedOnLicenseplate(licenseplate);
  const resAPK = await fetch(apkURL);
  const APKInfo = await resAPK.json();

  info.apk_keuringen = new Map();
  for (let i = 0; i < APKInfo.length; i++) {
    const apk = APKInfo[i];
    const apk_keuring = info.apk_keuringen.get(apk.meld_datum_door_keuringsinstantie) || [];
    apk_keuring.push(apk);
    info.apk_keuringen.set(apk.meld_datum_door_keuringsinstantie, apk_keuring);
    APKInfo[i].description = gebreken.get(apk.gebrek_identificatie);
  }
